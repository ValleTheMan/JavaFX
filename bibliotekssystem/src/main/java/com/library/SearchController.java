    ...
    public void initialize() {
        ...
        resultListView.setOnMouseClicked(e -> {
            if (e.getClickCount() == 2) {
                Item selectedItem = resultListView.getSelectionModel().getSelectedItem();
                if (selectedItem != null) {
                    javafx.fxml.FXMLLoader loader = SceneManager.switchScene("Item-view.fxml", "Detaljer - " + selectedItem.getTitle());
                    if (loader != null) {
        ...
        loginButton.setOnAction(event -> {
         SceneManager.openLogin("Search_Item.fxml");
        });
    }
